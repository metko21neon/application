...
const COIN_LIST: any[] = [
  ...
  },
  {
    ...
    wallets: [
      {
        address: '0xDc56ed4117C7d1350e9c7164220f6342F85BEA95',
        quantity: 212.242516343787086561
      }
    ]
  },
  {
...
export class AppService {
  ...
  private investStatisticState: InvestStatisticInterface = STATE;
  ...
  getCoinByAddress(address: string): CoinInterface | undefined {
  ...
  getCoinListSettings(): Observable<any> {
    return of(CRYPTO).pipe(
      tap((response: any) => this.setCryptoListState(response.default)),
    );

    // return this.http.get<ResponseInterface>('/api/users').pipe(
    //   map((response: ResponseInterface) => {
    //     return response.data.map((coinData: CoinDataInterface) => {
    //       return {
    ...
    //     });
    //   }),
    //   tap((response: any) => this.setCryptoListState(response)),
    // );
  }

  getCoinDataBySymbol(coin: CoinInterface): any {
    return this.cryptoListState.find((crypto: any) => {
      if (coin?.token_address) {
        return crypto?.token_address?.toLowerCase() === coin.token_address.toLowerCase();
      }

      return crypto?.symbol === coin?.symbol;
    });
  }
    ...
    return (of(COIN_LIST) as Observable<any[]>).pipe(
      tap((coinList: CoinInterface[]) => {
        coinList.map((coin: CoinInterface) => {
          const coinData = this.getCoinDataBySymbol(coin);
          ...
          this.setCoinCurrencyResultProperty(coin, coinData);
  ...
  private getRoundedValue(value: number): number {
    ...
    if (value > 0.0000001 && value < 0.0001) {
  ...
  private setCoinTotalInCurrencyProperty(coin: CoinInterface, coinData: any): void {
    const totalInCurrency = coin.quantity * coinData?.price;

    coin.totalInCurrency = this.getRoundedValue(totalInCurrency);
  }

  private setCoinCurrencyResultProperty(coin: CoinInterface, _: any): void {
    const currencyResult = coin.totalInCurrency - coin.investedAmount;

    coin.currencyResult = this.getRoundedValue(currencyResult);
  }

  private setCoinInvestedAmountProperty(coin: CoinInterface): void {
    const investedAmount = coin?.history?.reduce((acc: number, curr: CryptoHistoryInterface) => {
      ...
      if (curr.action === CoinHistoryActionEnum.TRANSFER) {
        return acc - curr.fee! * curr.averagePrice!;
      }
    ...
    coin.investedAmount = this.getRoundedValue(investedAmount);
  }

  private setCoinPercentResultProperty(coin: CoinInterface, coinData: any): void {
    const priceDifference = (coinData?.price - coin.averagePrice);
    const percentageResult = (priceDifference / (coin.averagePrice / 100));

    coin.percentageResult = coin.history?.length ? this.getRoundedValue(percentageResult) : 0;
  }

  private setCoinAveragePriceProperty(coin: CoinInterface): void {
    const averagePrice = calculateAveragePrice(coin);

    coin.averagePrice = this.getRoundedValue(averagePrice);
  }
  ...
  private setCoinPriceProperty(coin: CoinInterface, coinData: any): void {
    coin.price = this.getRoundedValue(coinData?.price);
  }

  private setCoinRankProperty(coin: CoinInterface, coinData: any): void {
  ...
  private setInvestStatisticState(coinList: CoinInterface[]): void {
    coinList.map((coin: CoinInterface) => {
      ...
      this.investStatisticState.totalInCurrency += coin.totalInCurrency;
    });
